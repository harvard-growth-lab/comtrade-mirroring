...
# complexity table
class Complexity(AtlasCleaning):
    NOISY_TRADE = {
    ...
    def load_data(self):
        self.aux_stats = pd.read_csv(
            os.path.join(self.static_data_path, "auxiliary_statistics.csv"), sep="\t"
        )
        self.reliable_exporters = pd.read_stata(
            os.path.join(self.static_data_path, "obs_atlas.dta")
        )
        ...
        if self.product_class == "SITC":
            ...
            self.df = pd.read_parquet(
                f"data/processed/{self.download_type}_{self.product_class}_{self.year}_country_country_product_year.parquet"
            )
