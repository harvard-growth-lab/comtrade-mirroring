	...
	quietly {
			...
			*-------------------------------------------------------------------------------
			...
			preserve
					...
					collapse (sum) tradevalue, by(exporter importer commoditycode)
					recast float tradevalue, force
			...
			*-------------------------------------------------------------------------------
			...
			preserve
				...
				collapse (sum) val99 VR , by(exporter )
				gen ratio99 = val99 / VR
				//gsort -ratio99
				levelsof exporter if ratio99>1/3, local(ctry2drop)
			restore
			*
