...
class GrowthProjectionsDataLoader(_AtlasCleaning):
        ...
        def load_natural_resources(self):
            ...
            df.loc[df.nrproducts==1, "net_exports"] = (df["export_value"] - df["import_value"]).clip(lower=0)
            
            df['total_exports'] = df.groupby(['year', 'exporter'])['export_value'].transform('sum')
            ...
            df['avubiquity'] = df.groupby(['year', 'exporter'])['avubiquity'].transform('sum')          
               
            df =  df.groupby(["year","exporter"]).agg({"net_exports":"sum", "eci":"first", "oppval":"first", "inatlas":"first", "diversity":"first", "avubiquity":"first"}).reset_index()
        ...
        def load_wdi_data(self):
            ...
            """
            return pd.read_csv(Path(self.atlas_common_path) / "wdi_indicators" / "data" / "wdi_data.csv")
        
        
        def load_population_forecast_data(self):
            """
            keep year idc iso ny_gdp_pcap_kd eci inatlas oppval 
            """
            cols = {'Year': "year", 
            ...
            pop_df = pd.read_excel(io.BytesIO(response.content), sheet_name='Estimates', header=16, usecols=cols.keys())
            pop_df = pop_df.rename(columns=cols)
            projections_df = pd.read_excel(io.BytesIO(response.content), sheet_name='Medium variant', header=16, usecols=cols.keys())
