...
class DataLoader(_AtlasCleaning):
    ...
    columns = ["Year", "Aggregate Level", "Trade Flow Code",
                "Reporter", "Reporter ISO", "Partner", "Partner ISO",
                "Commodity Code", "Qty Unit Code", "Qty", "Trade Value (US$)"]
    rename_cols = {
        'Year': "year", 
    ...
    def get_comtrade(self, year):
        ...
        """
        df = pd.read_csv(path.join(self.raw_data_path, f"{self.product_classification}_{year}.zip"),
                         compression="zip", 
